...
(function(){
		...
		switch(param.type){
			case 0:
				save2file(param.text, param.filetype || 'js', function(file){
					console.log(file);
					compress(file.path, file.filename, file.filetype, 'input');
				});
			break;
			case 1:
				...
				for(var i in param.files){
					...
					compress(file.path, file.filename, file.filetype, file.oname);
			...
			case 2:
				...
				for(var i in param.url){
					...
					compress(file.path, file.filename, file.filetype, oname);
				});
				}
			break;
			default:
	...
	//for download
	function compress(path, filename, filetype, oname){
		var retmsg = {};
		console.log('compress ', path, filename, filetype);
		var command = 'java -jar ./tools/yuicompressor-2.4.7.jar --type ' + filetype + ' -o ' + ' ./tmp/' + filename + '-' + oname + '.min.' + filetype + ' ' + path; 
		console.log(command);
		child = exec(command, function(error, stdout, stderr){
			if(error !== null){
				console.log('exec error:' + error);
